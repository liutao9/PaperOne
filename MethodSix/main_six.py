...
def main():
    ...
    ct, ori, spac = MethodOur.function.load_itk("../Data/test2.mhd")
    V0 = [0.77, -0.633, 0.0]
    f = open('result/result1.txt', 'w')
    f_time_mean = open('result/timeAndMean.txt', 'w')
    #'''
    ...
    while MethodOur.function.stop(np.array(current_A), end_point, 15):
        ...
        image = cv2.GaussianBlur(image, (5, 5), 1.5)
        count += 1
        print("the area is {0},the count is: {1}, {2},  {3}".format(np.sum(image) / 255 * x_scale * y_scale, count, best_V, current_A))
