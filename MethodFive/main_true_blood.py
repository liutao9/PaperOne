...
def main():
    ...
    V0 = [-0.67479813, 0.31554728, 0.66714121]      #### 初始化，根据我们自己的算法，选取最合适的方向进行初始化
    #'''
    ...
    while MethodOur.function.stop(np.array(current_A), end_point, 15):
        ...
        #image = MethodOur.getImage.getImage(current_A, oldnorm, scale1, scale2, width, height, x_scale, y_scale, ct)
        ...
        #print(V0)
        print('{0} {1} {2}'.format(current_A, np.sum(image_pre) / 255 * x_scale, current_i))
        f.write('{0}\n'.format(np.sum(image_pre) / 255 * x_scale))
